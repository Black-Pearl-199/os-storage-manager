...
export const MyDeleteManyButton = enhance((props) => {
    ...
    return (
        <button className={classNames('btn', 'btn-itech', disabled ? 'btn-itech-secondary' : 'btn-itech-primary')}
                disabled={disabled} onClick={handleClick}>
            <FontAwesomeIcon
                icon={faTrash}/>&nbsp;&nbsp;{translate('button.deleteMany', {resource_name: resourceName})}
        </button>
    );
});
