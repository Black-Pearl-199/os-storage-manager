...
import {crudGetAll, downloadCSV, CRUD_GET_MANY, GET_MANY, translate} from 'ra-core';
...
class ExportButton extends Component {
    ...
    render() {
        const {label, icon, translate, total, ...rest} = this.props;

        return (
            <Button variant='itech' size={'sm'}
                    className={`${total === 0 ? `btn-itech-secondary` : `btn-itech-primary`} btn-itech-fixed`}
                    disabled={total === 0}
                    onClick={this.handleClick}
                    {...sanitizeRestProps(rest)}>
                <FontAwesomeIcon icon={faDownload}/> &nbsp;{translate(label)}
            </Button>
        );
    }
}

const enhance = compose(translate, connect());

export const MyExportButton = enhance(ExportButton); // inject redux dispatch
