...
const enhance = compose(connect(mapStateToProps, { ITCrudGetList, showNotification, changeListParams }), translate);

const sanitizeRestProps = ({
    showNotification, initFilter, permissions, pagination, options, query, params, ids, data, total, version, location, history, match, basePath, hasList, hasCreate, hasEdit, hasShow, loadedOnce, selectedIds, isLoading, ITCrudGetList, defaultSort, changeListParams, ...rest
}) => rest;
...
class FilterBox extends Component {
    ...
    constructor(props) {
        ...
        const query = Object.keys(props.query).length > 0 ? props.query : hasCustomParams(props.params) ? { ...props.params } : { filter: props.initFilter || {} };
        const { filter } = query;
    ...
    onSubmit = debounce((e, firstInit) => {
        ...
        const {
            resource, ITCrudGetList, initFilter, permanentFilter, defaultSort, convertValue, changeListParams
        } = this.props;
        let filter = {};
        let { form } = this.state;
    ...
    resetFilter = () => {
        const resetState = {};
        const { form } = this.state;
        Object.keys(form).forEach((name) => { resetState[name] = undefined; });
    ...
    render = () => {
        const {
            hasClear, children, translate, className, buttonClasses, initData, defaultSort, permanentFilter, isLoading, ...rest
        } = this.props;
        const sanitizeProps = { ...sanitizeRestProps(rest), onInputChange: this.onChange };
        ...
        // 'card', 'panel-itech',
        return (
            <form onSubmit={this.formEnter} ref={this.formRef} className={classNames('row', 'my-1', className)}>
