            ...
            {choices.map(choice => {
                ...
                return (
                    <div key={choice.id}
                        ...
                        <input className='custom-control-input' type={type} name={name} id={id}
                               checked={inputValue === choice.value} onChange={onChange} value={choice.value}/>
                        <label htmlFor={id}
                               className='custom-control-label no-select'>{skipFormat ? choice.name : translate(choice.name)}</label>
                    </div>
                )
            })}
        </div>
